...
public class ValueMappingConfig {
    ...
    private final static Logger logger = LoggerFactory
        .getLogger(ValueMappingConfig.class);
    ...
    public void onChange(ConfigChangeEvent changeEvent) {
        logger.info("[op:onChange] valueMapping userName={} userSet={}",
            userName, gson.toJson(userSet));
    }

    protected static void setUserName(
        @Value("${userName:abc}") String userName) {
        logger.info("[op:setUserName] userName={}", userName);
    }

    @ValueMapping("${userJsonStr}")
    private void setUser(ValueMappingUserBean user) {
    ...
    protected void setAll(@Value("${userName:abc}") String userName,
        ...
        @ValueMapping(value = "${userJs?n*}", collector = ValueMappingAntMatchCollector.class) List<ValueMappingUserBean> userList,
        @ValueMapping(value = "${userXmlStr}", parser = ValueMappingXmlBeanParser.class) ValueMappingUserBean xmlUser) {
        logger.info("[op:setAll] userName={} user={} xmlUser={} userList={}",
            userName, gson.toJson(user), gson.toJson(xmlUser),
