...
public class RemoteConfigLongPollServiceTest extends ComponentTestCase {
  ...
  @Mock
  private HttpResponse<ApolloConfigNotification> pollResponse;
  @Mock
  ...
  public void testSubmitLongPollNamespaceWith304Response() throws Exception {
    ...
    doAnswer(new Answer<HttpResponse<ApolloConfigNotification>>() {
      @Override
      public HttpResponse<ApolloConfigNotification> answer(InvocationOnMock invocation)
          throws Throwable {
        try {
        ...
        assertTrue(request.getUrl().contains(someServerUrl + "/notifications?"));
        assertTrue(request.getUrl().contains("appId=" + someAppId));
        assertTrue(request.getUrl().contains("cluster=" + someCluster));
        assertTrue(request.getUrl().contains("namespace=" + someNamespace));

        longPollFinished.set(true);
        return pollResponse;
      }
    }).when(httpUtil).doGet(any(HttpRequest.class), eq(ApolloConfigNotification.class));
    ...
    doAnswer(new Answer<HttpResponse<ApolloConfigNotification>>() {
      ...
      public HttpResponse<ApolloConfigNotification> answer(InvocationOnMock invocation)
        ...
        if (counter.incrementAndGet() == 1) {
          ...
          HttpRequest request = invocation.getArgumentAt(0, HttpRequest.class);
          assertTrue(
              request.getUrl().contains("namespace=" + someNamespace + "%2B" + anotherNamespace));

          when(pollResponse.getStatusCode()).thenReturn(HttpServletResponse.SC_OK);
          when(pollResponse.getBody()).thenReturn(anotherNotification);
        } else {
          when(pollResponse.getStatusCode()).thenReturn(HttpServletResponse.SC_NOT_MODIFIED);
  ...
  public void testAssembleLongPollRefreshUrl() throws Exception {
    ...
    assertTrue(longPollRefreshUrl.contains(someServerUrl + "/notifications?"));
  ...
  public void testAssembleLongPollRefreshUrlWithMultipleNamespaces() throws Exception {
    ...
    String longPollRefreshUrl =
        remoteConfigLongPollService
            .assembleLongPollRefreshUrl(someUri, someAppId, someCluster,
                someNamespace + "+" + anotherNamespace, null, null);

    assertTrue(longPollRefreshUrl.contains(someServerUrl + "/notifications?"));
    assertTrue(longPollRefreshUrl.contains("appId=" + someAppId));
    assertTrue(longPollRefreshUrl.contains("cluster=someCluster%2B+%26.-_someSign"));
    assertTrue(
        longPollRefreshUrl.contains("namespace=" + someNamespace + "%2B" + anotherNamespace));
